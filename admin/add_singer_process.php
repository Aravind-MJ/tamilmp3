...
if($_POST){
    ...
    $uploadpath = 'singers/';  // directory to store the uploaded files
        ...
        if ($err == '') {
            ...
            */
            if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadpath)) {
                ...
                echo '<script> window.location.href="add_singer.php?status=2"; </script>';
            }
        } else {
            //header('Location: add_movie.php?status='.$err.'');
            echo '<script> window.location.href="add_singer.php?status=' . $err . '"; </script>';
        }
    }
    
    $query = sprintf("INSERT INTO singers SET name='%s',image='%s'", $name,$file);
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    
    echo '<script> window.location.href="add_singer.php?status=0"; </script>';
