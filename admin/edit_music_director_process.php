...
if($_POST){
    ...
    if (mysqli_num_rows($result) > 0) {
        echo '<script> window.location.href="view_music_director.php?status=Name Unchanged or Another same Name Exists"; </script>';
        die();
    }
    
    $query = sprintf("UPDATE music_directors SET name='%s' WHERE id=%d", $name,$id);
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
        ...
        if ($err == '') {
            ...
            */
            if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadpath)) {
                $query = sprintf("UPDATE music_directors SET image='%s' WHERE id=%d", $file, $id);
                $result = mysqli_query($link, $query) or die(mysqli_error($link));
            } else {
                //header('Location: add_movie.php?status=2');
                echo '<script> window.location.href="view_music_director.php?status=2"; </script>';
            }
        } else {
            //header('Location: add_movie.php?status='.$err.'');
            echo '<script> window.location.href="view_music_director.php?status=' . $err . '"; </script>';
        }
    }
    
    echo '<script> window.location.href="view_music_director.php?status=0"; </script>';
