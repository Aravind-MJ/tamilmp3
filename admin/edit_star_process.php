...
if ($_POST) {
    ...
    $query = sprintf("SELECT * FROM stars WHERE name='%s'", $name);
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    if (mysqli_num_rows($result) > 0) {
        echo '<script> window.location.href="view_stars.php?status=Name Unchanged or Another same Name Exists"; </script>';
        die();
    }

    //Edit Name    
    $query = sprintf("UPDATE stars SET name='%s' WHERE id=%d", $name, $id);
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    ...
    if (isset($_FILES['image']) && strlen($_FILES['image']['name']) > 1) {
        ...
        if ($err == '') {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadpath)) {
                ...
                echo '<script> window.location.href="view_stars.php?status=2"; </script>';
            }
        } else {
            //header('Location: add_movie.php?status='.$err.'');
            echo '<script> window.location.href="view_stars.php?status=' . $err . '"; </script>';
        }
    }

    echo '<script> window.location.href="view_stars.php?status=0"; </script>';
}
