...
if($_POST){
    ...
    if (mysqli_num_rows($result) > 0) {
        echo '<script> window.location.href="add_star.php?status=1"; </script>';
        die();
    }
        ...
        if ($err == '') {
            ...
            */
            if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadpath)) {
                ...
                echo '<script> window.location.href="add_star.php?status=2"; </script>';
            }
        } else {
            //header('Location: add_movie.php?status='.$err.'');
            echo '<script> window.location.href="add_star.php?status=' . $err . '"; </script>';
        }
    }
    
    $query = sprintf("INSERT INTO stars SET name='%s',image='%s'", $name,$file);
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    
    echo '<script> window.location.href="add_star.php?status=0"; </script>';
