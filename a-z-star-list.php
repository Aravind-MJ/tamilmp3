    ...
    <div class="container">
        <div class="row">
            <?php include_once('left-menu.php'); ?>
                ...
                <?php
                $star_id      = isset($_GET['id']) ? $_GET['id'] : 0;
                $sqlMovies    = sprintf("SELECT m.id, m.name FROM movies m JOIN starred_by s ON m.id = s.star_id WHERE m.name LIKE '%s' AND s.star_id = '%s'", 'A'.'%',$star_id);
...
<script>
    $(function() {
        $('.alphabets').on('click', function() {
            ...
            $.get( "ajax-pages/ajax-a-z-list-star.php?alphabet="+alphabet+"&star_id="+star_id)
                .done(function( data ) {
