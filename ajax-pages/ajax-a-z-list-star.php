...
$star_id      = isset($_GET['star_id']) ? $_GET['star_id'] : '0';
$sqlMovies    = sprintf("SELECT m.id, m.name FROM movies m JOIN starred_by s ON m.id = s.star_id WHERE m.name LIKE '%s' AND s.star_id = '%s'", $alphabet.'%',$star_id);
